...
// 1) Bound: An Index for increase the search process of point containment.
// 2) Decoded: Represents the polygon on its decoded value, the decoded value
...
// FromJSONFile creates a polygon from a json file.
// The structure of the json file is defined by the JSONData struct.
func FromJSONFile(path string) Polygon {
	type JSONData struct {
		Coordinates [][]float64
	}
	file, err := ioutil.ReadFile(path)
	if err != nil {
		log.Println("Impossible read the json polygon file", err)
	}
	var data JSONData
	err = json.Unmarshal(file, &data)
	if err != nil {
		log.Println("Error decoding the polygon from the json file:", path, err)
	}
	return FromCoordinates(data.Coordinates)
}

// FromCoordinates creates a polygon from a set of flat coordinates.
...
// CoverageCell returns the cell that covers the while polygon.
